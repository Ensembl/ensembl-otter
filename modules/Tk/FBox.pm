...
# BEGIN { warn "Sourcing patched Tk::FBox"; }

use strict;
use vars qw($VERSION $updirImage $folderImage $fileImage);

$VERSION = sprintf '4.%03d', 1;
...
sub Update {
    ...
    my $folder_icon = $folderImage->{$w->MainWindow};
    my $file_icon   = $fileImage->{$w->MainWindow};
    my $appPWD = _cwd();
    ...
    if (opendir(FDIR, $cwd)) {
        ...
        foreach my $f (sort $sortcmd readdir(FDIR)) {
            my $is_dir = -d "$cwd/$f";
            if ($is_dir) {
                # Always hide hidden directories
                next if $f =~ /^\./;
            } else {
                # Skip if we only want to see directories
                next if $type_dir;
            }
            if ($is_dir) {
                $icons->Add($folder_icon, $f);
            } else {
                if ($fltcb) {
                    # Skip any files which don't return true from the filter sub
                    next unless $fltcb->($w, $f, $cwd);
                }
                elsif (! $is_dir) {
                    # Skip any files which don't match the filter regex
                    next if $f !~ m!$flt!;
                }
                push @files, $f;
            }
...
# This is so we can use anonymous subs in the getOpenFile call to hide dot files
sub GetFileTypes {
    my $in = shift;

    my %fileTypes;
    foreach my $t (@$in) {
        push @{ $fileTypes{$t->[0]} }, (ref $t->[1] eq 'ARRAY' ? @{ $t->[1] } : $t->[1]);
    }
