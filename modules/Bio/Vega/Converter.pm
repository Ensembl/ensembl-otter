...
sub fill_transcript_AceText {
    ...
    if (my $translation = $tran->translation) {
        ...
        if ($strand == 1) {
            $ace->add_tag('CDS',
                rna_pos($tsct, $tran->coding_region_start),
                rna_pos($tsct, $tran->coding_region_end),
                );
        } else {
            $ace->add_tag('CDS',
                rna_pos($tsct, $tran->coding_region_end),
...
sub otter_to_ace {
    ...
    my $start_of_slice_in_chr = $slice->chr_start;
        ...
        # by the widest start/end combination they cover in the assembly
        #
    my %group_start = (); # hashes by contig_name
    my %group_end   = (); # ---------,,----------
    my %group_list  = (); # ---------,,----------
    ...
    $str .= ace_transcripts_locus_people($genes, $slice);
        ...
        # I don't think we ever use the (assembled) DNA of the slice anymore
    if($seq) {
        # Finally the dna
...
sub ace_transcripts_locus_people{
    ...
    my $str = ace_transcript_seq_objs_from_genes($genes, $slice, \%authors);
    $str .= "\n";
...
sub specific_gene_versions_to_XML {
    ...
    $xmlstr .= Bio::Vega::Converter::path_to_XML($chr, $chrstart, $chrend, $db->assembly_type, $path);
