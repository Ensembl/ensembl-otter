...
sub draw{
    ...
    my $label         = $search_window->Label(-text => "Use spaces to separate multiple terms")->pack(-side => 'top');
    ...
    # this bind is a bit of a bind. The $search_entry widget is implicitly passed to $doSearch as first arg.
    ...
    $search_entry->bind('<Return>' => [ sub { shift; $doSearch->(@_) }, $self]);

    # NEEDS THIS bind()!!!!!
    $search_entry->bind ('<Destroy>', sub { $self = undef; });

    ## radio buttons        
    my $radio_frame = $search_window->Frame()->pack(-side => 'top',
                                                    -pady => 5,
                                                    -fill => 'x'); 
    ...
    # NEEDS THIS bind()!!!!!
    $radio_frame->bind('<Destroy>', sub { $self = undef; });

    ## search cancel buttons
    my $search_cancel_frame = $search_window->Frame()->pack(-side => 'bottom', 
                                                            -padx =>  5,
                                                            -pady =>  5, 
                                                            -fill => 'x',
                                                            );   
    ...
    my $context_entry       = $search_cancel_frame->Entry(-width        => 5,
                                                          -relief       => 'sunken',
                                                          -borderwidth  => 2,
    ...
    # NEEDS THIS bind()!!!!!
    $search_cancel_frame->bind('<Destroy>', sub { $self = undef; });
