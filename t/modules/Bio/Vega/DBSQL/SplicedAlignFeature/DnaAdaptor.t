...
my $saf_da_module;
BEGIN {
    $saf_da_module = 'Bio::Vega::DBSQL::SplicedAlignFeature::DnaAdaptor';
    use_ok($saf_da_module);
}
critic_module_ok($saf_da_module);
...
my $dsaf_adaptor = $vega_dba->get_DnaSplicedAlignFeatureAdaptor;
isa_ok($dsaf_adaptor, 'Bio::Vega::DBSQL::SplicedAlignFeature::DnaAdaptor');
...
# Mostly cut'n'paste from EnsEMBL dnaAlignFeatureAdaptor.t
my $start      = 100_100;
my $end        = 100_200;
my $strand     = 1;
my $hstart     = 10;
...
$dsaf_adaptor->store($feat);

ok($feat->is_stored($vega_dba), 'now stored');

my $dbID = $feat->dbID();
my $r_feat = $dsaf_adaptor->fetch_by_dbID($dbID, 'contig');
