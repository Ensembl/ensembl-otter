...
use FlyBaseSetupConf;


my $host   = $FB_DB_HOST ; 
my $port   = $FB_DB_PORT;
my $dbname = $FB_DB_NAME;
my $dbuser = $FB_DB_USER;
my $dbpass = $FB_DB_PASS;
my $help;
my $cs_name = $FB_COMPONENT_NAME;
my $cs_version = $FB_ASSEMBLED_VERSION;
my $default = 0;
my $sequence_level = 1;
my $agp;
my $fasta;
...
print "=========debug=========\n" ;
print "coord_system_name ==$cs_name\n";
print "coord_system_version ===$cs_version======\n";
print "rank ----$rank---\n" ;
...
if(!$cs_name || (!$fasta  && !$agp)){
  ...
  if($agp) {
	print STDERR "motherfucker : -agp_file $agp\n";
  }
  if($fasta) {
	print STDERR "fatherfucker: -fasta_file $fasta\n";
  }
