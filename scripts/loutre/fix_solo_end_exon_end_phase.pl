...
# FIXME: copied from, and lots of duplication with, fix_end_exon_end_phase.pl

use strict;
use warnings;
use 5.010;

package Bio::Otter::Script::FixSoloEndExonEndPhase;
...
sub ottscript_options {
    return (
        ...
        dataset_class         => 'Bio::Otter::DataSet::FixSoloEndExonEndPhase',
        gene_class            => 'Bio::Otter::Gene::FixSoloEndExonEndPhase',
...
sub do_gene {
    ...
    foreach my $ts ( @{$gene->get_all_Transcripts} ) {
        ...
        my $status = '';
        if (       ($strand ==  1 and $tl_high == $ee->end)
            ...
            my $actual_end_phase = $coding_length % 3;

            if ($actual_end_phase != $ee->end_phase) {

                $status = sprintf("%s: correcting bad end_phase, was %d, now %d",
                                  $ee->stable_id, $ee->end_phase, $actual_end_phase);
...
package Bio::Otter::DataSet::FixSoloEndExonEndPhase;
use Moose;
...
around 'gene_sql' => sub {
    ...
    $sql =~ s/__EXTRA_CONDITIONS__/
              AND ts.is_current = 1
              AND tl.start_exon_id = tl.end_exon_id
              AND srh.value = 0 /;
    $sql =~ s/__GROUP_BY__/
              GROUP BY g.gene_id /;
    return $sql;
...
package Bio::Otter::Gene::FixSoloEndExonEndPhase;
use Moose;
...
Bio::Otter::Script::FixSoloEndExonEndPhase->import->run;
