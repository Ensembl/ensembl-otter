...
sub delete_xrefs {
  ...
  # filter since some might be shared)
  $sql = qq(
        SELECT
                x.xref_id
        FROM
  ...
  # finally get the xrefs to delete...
  my $xref_string = join(",", map { $_->[0] } @xrefs) || 0;

  # ...and delete all orphan object xrefs
  ...
  # delete from go_xref
  $support->log_stamped("Deleting from go_xref...\n", 1);
  $sql = qq(DELETE FROM go_xref where object_xref_id IN ($object_xref_string));
  $num = $dbh->do($sql);
  $support->log_stamped("Done deleting $num entries.\n", 1);
