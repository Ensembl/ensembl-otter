...
cleanup_GD_genes.pl - identify CORF genesfor deletion
...
This script identifies all CORF genes in a Vega database and dumpes stable IDs to
...
foreach my $gsi (keys %corf) {
	$c1++;
	$support->log_verbose("$c1. $gsi (".$corf{$gsi}.")\n",1);
}
...
foreach my $gsi (keys %GD_to_delete) {
	$c5++;
	$support->log_verbose("$c5. $gsi (".$GD_to_delete{$gsi}.")\n",1);
}

#report on GD genes that will be deleted as  CORF
$support->log_verbose("\nThe following CORF genes do not overlap Havana and are priorities for reannotation:\n");
foreach my $gsi (keys %corf_to_annotate) {
	$c2++;
	$support->log_verbose("$c2. $gsi (".$corf_to_annotate{$gsi}.")\n",1);
}
...
foreach my $gsi (keys %GD_to_annotate) {
	$c6++;
	$support->log_verbose("$c6. $gsi (".$GD_to_annotate{$gsi}.")\n",1);
}
