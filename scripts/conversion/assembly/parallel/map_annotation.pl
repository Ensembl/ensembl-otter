...
$parallel->job_params({ memory => 3000 });
$parallel->register(\@chrs);

RUN: 1;

CHROM:
foreach my $V_chr ($parallel->mytasks) {
...
POST:
foreach my $data ($parallel->postrun) {
  next unless defined $data;
  $parallel->merge_totals(\%trans_numbers,$data->{'trans_numbers'});
  $parallel->merge_totals(\%stat_hash,$data->{'stat_hash'});
}
